...
async def edit_entry(
    ...
    await entries_coll.update_one(
        {"_id": entry_id},
        {"$set": new_data, "$push": {"history": new_history_data}},
        upsert=True,
    )
    return entry_id
...
async def new_editor_session(discord_id):
    ...
    await sessions_coll.insert_one(
        {"_id": sid, "discord": discord_id, "time": datetime.now()}
    )
    return sid
...
async def search_entries(
    query, limit=10, search_id=True, page=0, discord_id=None, unlisted=False
):
    print("searching")
    found = []
...
async def get_entries(
    sort, limit=20, page=0, query=None, discord_id=None, unlisted=False
):
    # match = {'$match': {'unlisted': {'$ne': True}}}
    ...
    if sort == "relevant" and query:
        found = await search_entries(
            query, limit=limit, page=page, discord_id=discord_id, unlisted=unlisted
        )
        return found
...
async def set_personal_entry(discord_id, entry_id):
    ...
    async for entry in entries_coll.find({"owner_id": discord_id}):
        await entries_coll.update_one(
            {"_id": entry["_id"]}, {"$set": {"owner_id": None}}
        )
...
async def get_random_entry():
    cursor = entries_coll.aggregate(
        [{"$match": {"unlisted": {"$ne": True}}}, {"$sample": {"size": 1}}]
    )
    found = []
    async for entry in cursor:
        found.append(entry)
