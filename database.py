...
async def edit_entry(title,
	...
	await entries_coll.update_one({'_id': entry_id}, {
	    '$set': new_data,
	    '$push': {
...
async def search_entries(query,
                         limit=10,
                         search_id=True,
                         page=0,
                         discord_id=None,
...
async def get_entries(sort,
	...
	if sort == 'relevant' and query:
		found = await search_entries(query,
		                             limit=limit,
		                             page=page,
		                             discord_id=discord_id,
		                             unlisted=unlisted)
		return found
	cursor = entries_coll.find(match)
...
async def set_personal_entry(discord_id, entry_id):
	...
	await users_coll.update_one({'_id': discord_id}, {'$set': user_data},
	                            upsert=True)
	async for entry in entries_coll.find({'owner_id': discord_id}):
		await entries_coll.update_one({'_id': entry['_id']},
		                              {'$set': {
		                                  'owner_id': None
		                              }})
	await entries_coll.update_one({'_id': entry_id},
