...
def check_name_with_discrim(ctx, arg):
    member = discord.utils.find(
        lambda m: str(m).lower() == arg.lower(), get_channel_members(ctx.channel.id)
    )
    return member
...
def check_name_without_discrim(ctx, arg):
    member = discord.utils.find(
        lambda m: m.name.lower == arg.lower(), get_channel_members(ctx.channel.id)
    )
    return member
...
def check_name_contains(ctx, arg):
    member = discord.utils.find(
        lambda m: arg.lower() in m.name.lower(), get_channel_members(ctx.channel.id)
    )
    return member
...
def html_to_markdown(inputted, prev=None):
    ...
    new_string = re.sub(
        r"<(?:\s*)(i|em)(?:\s*)(?:[\S\s]*)>(.+?)<(?:\s*)\/\1>>", r"*\1*", new_string
    )
    ...
    for found in list(
        ...
        if href[0] == "/":
            href = BASE_URL + href
        new_string = (
            new_string[:span_start] + f"[{href_text}]({href})" + new_string[span_end:]
        )
...
async def before_show_text(inputted):
    ...
    for found in list(re.finditer(r"{{\s{0,}?(.{1,}?)\s{0,}?}}", new_string))[::-1]:
        ...
        if variable in variables:
            ...
            new_string = (
                new_string[:span_start] + variable_output + new_string[span_end:]
            )
...
def get_top_editors(history, limit=5):
    ...
    return {
        ...
        for editor in dictsort(top_editors)[:limit]
        if top_editors[editor]
    }
