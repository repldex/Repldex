...
async def help(message, *args):
	...
	if message.author.id in EDITOR_IDS:
		...
		if message.author.id in ADMIN_IDS:
			commands[
			    'link <user mention> <article>'] = 'Manually link non-editors to entries (admin only)'
			commands[
			    'view_selfentry <user mention>'] = 'View a users selfentry (admin only)'
			commands[
			    'who_is_the_leader'] = 'tells you who the supreme leader is (admin only)'
			commands[
			    'userinfo <user mention>'] = 'get info on the mentioned user (admin only)'
			commands[
			    'unlist <article id>'] = 'Toggles unlisting of entry (admin only)'
...
async def create_entry_embed(entry_data, author_id=None, raw_entry=False):
	if (not raw_entry):
		...
		view_url = view_url
		embed = discord.Embed(title=entry_data['title'],
		                      description=content,
		                      url=view_url)
		...
		return utils.embed_from_dict(entry_data,
		                             color=0x00ff00,
...
async def search_entries(message, *args):
	...
	if found:
		...
		content = 'No results'
	embed = discord.Embed(title=f'Results for {search_query}',
	                      description=content)
	# if found:
	...
	if found:
		...
		embed = await create_entry_embed(found[emoji_pos],
...
async def show_raw_entry(message, *args):
	...
	found = await database.search_entries(search_query, limit=1)
	if found:
		data = await create_entry_embed(found[0],
		                                author_id=message.author.id,
		                                raw_entry=True)
		...
		if not entry_id:
			if message.author.id not in EDITOR_IDS:
				return await message.send(
				    "You don't have a personal entry set yet. An admin needs to set one for you"
				)
			else:
...
async def user_info(message, member: utils.Member):
	...
	embed = discord.Embed(title='user info',
	                      description=f'info on <@{member.id}>',
	                      color=0x00ff00)
	editor = False
...
async def unlist(message, entry_id):
	...
	await database.edit_entry(title=entry_data['title'],
	                          entry_id=entry_id,
	                          content=entry_data['content'],
	                          unlisted=unlist)
	embed = discord.Embed(title='toggle unlist',
	                      description='toggling entry being listed/unlisted',
	                      color=0x00ff00)
	...
	embed.add_field(name='entry link',
	                value=BASE_URL + '/entry/' + entry_id,
	                inline=True)
...
async def new_entry(message, *args):
	...
	edit_url = edit_url
	if message.author.id in EDITOR_IDS:
		embed = discord.Embed(title="Write " + search_query,
		                      description=f'[Click here to write it!]({edit_url})')
		found = await database.search_entries(search_query, limit=1)
		if found:
			embed.set_footer(
			    text=
			    'Alert: There may be an entry with the same/similar name or topic.')
	else:
