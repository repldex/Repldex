...
async def help(message, *args):
    ...
    if message.author.id in EDITOR_IDS:
        ...
        if message.author.id in ADMIN_IDS:
            commands[
                "link <user mention> <article>"
            ] = "Manually link non-editors to entries (admin only)"
            commands[
                "view_selfentry <user mention>"
            ] = "View a users selfentry (admin only)"
            commands[
                "who_is_the_leader"
            ] = "tells you who the supreme leader is (admin only)"
            commands[
                "userinfo <user mention>"
            ] = "get info on the mentioned user (admin only)"
...
async def create_entry_embed(entry_data, author_id=None, raw_entry=False):
    if not raw_entry:
        ...
        view_url = view_url
        embed = discord.Embed(
            title=entry_data["title"], description=content, url=view_url
        )
...
async def show_raw_entry(message, *args):
    ...
    found = await database.search_entries(search_query, limit=1)
    if found:
        data = await create_entry_embed(
            found[0], author_id=message.author.id, raw_entry=True
        )
...
async def user_info(message, member: utils.Member):
    if message.author.id not in ADMIN_IDS:
        return
    embed = discord.Embed(
        title="user info", description=f"info on <@{member.id}>", color=0x00FF00
    )
    editor = False
...
async def unlist(message, entry_id):
    ...
    embed.add_field(
        name="entry link", value=f"{BASE_URL}/entry/{entry_id}", inline=True
    )
...
async def new_entry(message, *args):
    ...
    if message.author.id in EDITOR_IDS:
        ...
        if found:
            embed.set_footer(
                text="Alert: There may be an entry with the same/similar name or topic."
            )
    else:
...
async def ping(message, *args):
    ...
    await message.send(
        embed=discord.Embed(title="Ping!", description=f"Bot ping is: `{int(ping)}ms`")
    )
